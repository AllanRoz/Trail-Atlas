...
export default function FilterSidebar({ filters, onChange }) {
  const states = useMemo(
    () => [...new Set(trails.map((t) => t.state))].sort(),
    [],
  );
  const parks = useMemo(
    () => [...new Set(trails.map((t) => t.nationalPark))].sort(),
    [],
  );
  ...
  return (
    <div className="flex flex-wrap items-start gap-x-8 gap-y-5 rounded-2xl bg-white p-5 shadow-card dark:bg-pine-700/40">
      ...
      <FilterGroup label="Route Type">
        {loopTypes.map((l) => (
          <Pill
            ...
            onClick={() =>
              onChange({ ...filters, loop: filters.loop === l ? null : l })
            }
          >
            {l}
          </Pill>
      ...
      <div className="min-w-[160px]">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-pine-700/50 dark:text-tan-100/50">
          Max Distance:{" "}
          <span className="font-mono text-pine-700 dark:text-tan-100">
      ...
      <div className="min-w-[160px]">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-pine-700/50 dark:text-tan-100/50">
          Max Elevation Gain:{" "}
          <span className="font-mono text-pine-700 dark:text-tan-100">
            {filters.maxElevation.toLocaleString()} ft
          </span>
        </h3>
        <input
          type="range"
          ...
          onChange={(e) =>
            onChange({ ...filters, maxElevation: Number(e.target.value) })
          }
          className="w-full accent-moss-500"
        />
      </div>
