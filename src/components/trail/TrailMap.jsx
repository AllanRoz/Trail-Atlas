...
export default function TrailMap({ trails, className = 'h-full w-full', zoom = 4, route = null }) {
  ...
  return (
    <div className={className}>
      <MapContainer
        {...(bounds ? { bounds, boundsOptions: { padding: [40, 40] } } : { center, zoom })}
      ...
      >
        ...
        {route && (
          <>
            <Polyline positions={route} pathOptions={{ color: '#B5562A', weight: 4, opacity: 0.85, lineCap: 'round' }} />
            ...
            <CircleMarker
              center={route[0]}
              radius={9}
              pathOptions={{ color: '#1F3A2E', weight: 3, fillColor: '#4B7455', fillOpacity: 1 }}
            >
